...
AIProject::DynamicSeparation::DynamicSeparation(Boid & i_character, Kinematic(&i_targets)[], const int & i_size, const float & i_threshold,
...
AIProject::DynamicSteeringOutput AIProject::DynamicSeparation::GetSteering()
{
	...
	{
		if (m_character.m_kinematic.id == m_targets[i].id)
			continue;

		ofVec2f direction = m_character.m_kinematic.position - m_targets[i].position;
		float distanceSquared = direction.lengthSquared();

		if (distanceSquared < m_threshold * m_threshold)
		{
			strength = min(m_constant / distanceSquared, m_maxAcceleration);
			steering.linearAcceleration += strength * direction.normalized();
		}	
	}
